...
function Read-AudiPackageDetail {
    ...
    if (-not (Test-Path -LiteralPath $PackagePath)) {
        ...
        return [pscustomobject]@{ Fields = $fields; Origin = $origin; Generation = $null
                                  ScriptPath = $null; DocumentPath = $null; Notes = @($notes)
                                  OperatingSystems = @() }
    }
...
function Get-AudiIntegrationPlan {
    ...
    # The SoftIdent rule is off by default: Audi detect on the branding key
    # alone. A second rule means a package that is installed but whose vendor
    # key moved reads as not installed, and reinstalls. Turn it back on with
    # enabled="true" on <SoftIdentDetection> - the rule itself is still built
    # and tested, only not used.
    $useSoftIdent = $false
    if ($defaults.SoftIdentDetection.HasAttribute('enabled')) {
        $useSoftIdent = [bool]::Parse($defaults.SoftIdentDetection.enabled)
    }

    $softIdentParts = $(if ($useSoftIdent) { Split-AudiSoftIdent -SoftIdent $SoftIdent -Root $Root } else { $null })
    if ($softIdentParts) {
        $detectionRules.Add([pscustomobject]@{
            Source    = 'SoftIdent'
