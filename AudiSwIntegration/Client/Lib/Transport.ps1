...
function Initialize-AudiDropFolder {
    ...
    # The four STATE folders always exist - they are the structure, and the
    # collector expects them.
    $states = Get-AudiDropFolderPath -DropFolder $DropFolder -EnvironmentCode $EnvironmentCode
    foreach ($p in @($states.New, $states.Working, $states.Done, $states.Failed)) {
...
function Remove-AudiEmptyPackageFolder {
    ...
    if ((Split-Path -Leaf $Path) -in @('New', 'Working', 'Done', 'Failed')) { return }
