...
$script:SPDefaults = [ordered]@{
    ...
    UsePredecessorRepo = $true      # replace PredecessorPath (Get-PredecessorCandidates)
}
...
function Get-PredecessorCandidates {
    param($Parsed)
    $cfg = Get-SPConfig
    if ($cfg.Enabled -and $cfg.UsePredecessorRepo) {
        $cands = @(Get-SPPredecessorCandidates -Parsed $Parsed)
        foreach ($c in $cands) { if ($c.SPRelPath) { Register-SPPredecessor -LocalPath $c.FullName -SPRelPath $c.SPRelPath } }
        return $cands
    }
