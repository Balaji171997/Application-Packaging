...
$script:BuildStamp = 'GPF-2026-08-04.r58'  # shown in the WINDOW TITLE so you always know which build/pak you run
...
function Ensure-PublishModulesStaged {
    ...
    foreach ($d in 'Lib\ConfigurationManagerPrelive', 'Lib\PowerShell Module') {
...
function Initialize-Config {
    ...
    $defaults = [ordered]@{
        ...
        SnippetsPath         = ''
        Sccm                 = [ordered]@{
            SiteCode='XXX'; SiteServer='localhost'
            ContentShare='C:\temp\GPF-SccmContent'; DPGroup=''
        ...
        # BRAND PROFILE: which team flavour this deployment serves. Defaults = MTB (our team, current behaviour).
        # A brand copy (e.g. GPF GroupWrapper) ships the SAME exe+pak with its own settings.json overriding this block.
        Brand                = [ordered]@{
            Name             = 'MTB'                 # 'MTB' | 'GPF'
            TemplateRoot     = 'PSADT_Template'      # tool-relative template folder (GPF copy: 'PSADT_Template_GPF')
            OrderNumberLabel = 'RITM'                # UI label + script field ('AES' for GPF - from the AES-1-... request folder)
            OutgoingPrefix   = [ordered]@{}          # outgoing folder-name prefixes: @{ INA='Gpf'; VWG='Group package'; G1V='VW' } (empty = no prefix, MTB style)
            Features         = [ordered]@{ Sccm=$true; Intune=$true; Publish=$true }   # GPF: all $false (build ends at Create+Verify)
            Convert          = [ordered]@{           # v3->v4 conversion behaviour per brand
                MtbMappings     = $true              # Remove-Branding->Remove-MTBDetectionKey etc. (GPF: $false - their v4 keeps Set/Remove-Branding verbatim)
                VwgVarRename    = $true              # $VWG_*/$appName -> $adtSession.* (GPF: $false - their v4 Global bridge keeps them alive)
                RegWowHardcode  = $true              # $VWG_CurrentRegWow -> Wow6432Node hardcode (GPF: $false - defined in their v4 extensions)
                LogPathMain     = $true              # $configToolKitLogDir\$setuplogName -> $LogPathMain/$LogFileMain house style (GPF: $false)
                SoftIdentFormat = 'MTB'              # SoftIdent value syntax: MTB '[DisplayVersion = x]' | GPF '[DisplayVersion]=x'
            }
        }
    }
...
# ---------- Brand profile ----------
# Settings arrive as hashtables (defaults) OR PSCustomObjects (from JSON) - this reads a member from either.
function Get-PBMember {
...
# One brand value, deep-path via dots ('Convert.MtbMappings'). Missing anywhere -> the MTB default you pass.
function Get-PBBrand {
    param([string]$Path = 'Name', $Default = $null)
    $node = Get-Setting Brand
    foreach ($seg in ($Path -split '\.')) {
        $node = Get-PBMember -Object $node -Name $seg
...
function Test-PBConvertFlag {
    ...
    return [bool](Get-PBBrand -Path "Convert.$Flag" -Default $true)
}
