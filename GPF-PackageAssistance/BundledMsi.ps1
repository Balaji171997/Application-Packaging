...
$proc=$null; $runFile=$Installer; $runArgs=$InstallArgs
...
try { if($runArgs){ $proc = Start-Process -FilePath $runFile -ArgumentList $runArgs -PassThru -EA Stop } else { $proc = Start-Process -FilePath $runFile -PassThru -EA Stop } } catch { "launch failed: $($_.Exception.Message)" | Out-File "$Out\_log.txt" -Append }
