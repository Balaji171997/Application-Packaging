...
# Read the MSI CheckBox table (Orca's "CheckBox" view): each row = @{ Property; Value } - the property a UI checkbox
# drives and the value it sets when ticked (this is how license-agreement props like AGREETOLICENSE/IAGREE are found).
# Empty list when the MSI has no CheckBox table (COM OpenView throws on a missing table - caught + treated as empty).
function Get-MsiCheckBoxTable {
    param([Parameter(Mandatory)][string]$MsiPath)
    $rows = New-Object System.Collections.Generic.List[object]
    if (-not (Test-Path $MsiPath)) { return $rows }
    $installer = $null; $db = $null; $view = $null
    try {
        $installer = New-Object -ComObject WindowsInstaller.Installer
        $db   = $installer.OpenDatabase($MsiPath, 0)   # 0 = read-only
        $view = $db.OpenView('SELECT `Property`, `Value` FROM `CheckBox`')
        $view.Execute($null)
        while ($true) {
            $rec = $view.Fetch(); if (-not $rec) { break }
            $rows.Add([pscustomobject]@{ Property = $rec.StringData(1); Value = $rec.StringData(2) })
    ...
    } catch { }   # no CheckBox table (or unreadable) -> empty list; this is common and not an error
    finally {
        foreach ($o in @($view, $db, $installer)) { if ($o) { try { [Runtime.InteropServices.Marshal]::ReleaseComObject($o) | Out-Null } catch {} } }
        [GC]::Collect(); [GC]::WaitForPendingFinalizers()
    }
