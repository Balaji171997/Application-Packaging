...
$script:SccmDefaults = [ordered]@{
    SiteCode           = 'XXX'                       # GPF copy: SCCM feature is OFF - neutral placeholders, NO MTB values
    SiteServer         = 'localhost'
    ContentShare       = 'C:\temp\GPF-SccmContent'
    DPGroup            = '_ALL_DPs'
...
function Get-SccmFieldsFromPackage {
    ...
    $get = { param($f)
        ...
        if ($ms.Count) { $ms[$ms.Count-1].Groups[1].Value } else { '' }
    }
    ...
    if ($soft -match '(?i)VWG_CurrentRegWO?W') {
        ...
        $wow = if ($dm.Count) { $dm[$dm.Count-1].Groups[1].Value } elseif ("$arch" -match '(?i)x86') { 'WoW6432Node\' } else { '' }
        $soft = [regex]::Replace($soft, '(?i)\$\(\s*\$?VWG_CurrentRegWO?W\s*\)|\$VWG_CurrentRegWO?W', $wow.TrimEnd('\'))
        $soft = [regex]::Replace($soft, '(?<=[^:\\])\\{2,}', '\')   # collapse doubles left when the token resolved empty
    }
    ...
    # Bitness for the uninstall clause: from the key when present, else from the package arch.
    $is32 = if ($uninstKey) { $uninst32 } else { "$arch" -match '(?i)x86' }
...
function Connect-Sccm {
    ...
    if (-not (Get-Module ConfigurationManager)) {
        ...
        try { Import-Module $mod -ErrorAction Stop } catch {
            ...
            # The CONSOLE's module failed (execution policy still blocked, assembly clash, corrupt install...). Do NOT
            # give up on a console machine - fall back to our BUNDLED copy, which is self-contained.
            # ONLY when NO ConfigMgr assembly is already in this AppDomain: importing the bundled build alongside
            # already-loaded console assemblies CLASHES (mismatched versions of the same types) - which is the very
            # reason we prefer the console's module. Get-Module being null is NOT proof nothing loaded (a PARTIAL
            # import leaves assemblies in), so check the AppDomain directly.
            elseif ($installedMod -and $bundledMod -and (Test-Path $bundledMod) -and -not (Test-ConfigMgrAssemblyLoaded)) {
                ...
                # Console module failed while its assemblies ARE already loaded (substituting the bundled build would
                # clash), or there is no bundled copy. Fail cleanly with the real reason.
