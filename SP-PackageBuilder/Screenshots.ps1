...
function Invoke-ShortcutScreenshots {
    param([object[]]$Shortcuts, [Parameter(Mandatory)][string]$OutDir, [scriptblock]$OnLog,
          [int]$TimeoutSec = 45, [int]$SecondShotSec = 20, [string]$Title = 'Shortcut screenshots')   # wait up to TimeoutSec for ITS process window, shot 1, then watch up to SecondShotSec for a screen CHANGE -> +5s settle -> shot 2; no window -> verified tray / background
      ...
      foreach ($s in @($Shortcuts)) {
        ...
        try {
            ...
            if ($hwnd -ne [IntPtr]::Zero) {
                ...
                # Watch up to SecondShotSec for the screen to CHANGE (loading finished / a dialog appeared). When a
                # change shows up, give the window 5 MORE seconds to settle (bigger change fully drawn), THEN take
                # shot 2. No change in the whole window -> one screenshot is enough.
                & $log "  [shot] $($s.Name): watching up to ${SecondShotSec}s for the screen to change..."
                $changed = $false
                $chDeadline = (Get-Date).AddSeconds($SecondShotSec)
                while ((Get-Date) -lt $chDeadline) {
                    Start-Sleep -Milliseconds 1000
                    $bT = & $grabScreen $false
                    $isDiff = (& $differ $sig1 (& $sig $bT))
                    if ($bT) { $bT.Dispose() }
                    if ($isDiff) { $changed = $true; break }
                }
                if ($changed) {
                    & $log "  [shot] $($s.Name): screen changed - letting it settle 5s before the second screenshot."
                    ...
                    & $log "  [shot] $($s.Name): kept the second screenshot."
                } else {
                    $pk2 = $picked
                    & $log "  [shot] $($s.Name): screen unchanged within ${SecondShotSec}s -> one screenshot is enough."
                }
...
function New-PBShotsAgentScript {
    ...
    if (-not $tok) { $tok = "''" }
    $tpl = @'
# PBShots agent - generated by Package Companion. Runs ON the test machine, in the logged-on user's session.
...
function Invoke-RemoteShortcutShots {
    ...
    $taskName = 'PackageCompanionShots'
    $cim = $null
    try {
