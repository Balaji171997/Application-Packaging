...
4. SCCM + Intune integrations — BOTH live in STEP 4 (user's plan 2026-06-08). Studied the team's reference
   ...
   2026-07-10 BRAND SPLIT (user correction - supersedes the "AUDI variant" entries above):
     The other team is **GPF** (serves 3 target brands: INA=Audi, VWG=Group package, G1V=VW - user-selected outgoing
     prefix). Per user instruction the variant now lives in a **SEPARATE folder**: C:\Users\AW140\Downloads\
     GPF-PackageBuilder (own settings.json Brand=GPF, PSADT_Template_GPF, BrandGPF.ps1, no SCCM/Intune payload, ~30 MB,
     pak GPF-2026-07-07.r1, ALL TESTS PASSED incl. 16 GPF corpus asserts vs Downloads\OtherBrand).
     THIS folder (Downloads\files) is MTB-ONLY again: ALL brand-round edits REVERTED (Core/Predecessor/Mappings/Build/
     Assemble/GUI/Test-Build/Pack-Engine restored; BrandAudi.ps1 + PSADT_Template_AUDI removed) - verified by full
     Test-Build ALL PASS = r180 behaviour. Cleanup: junk removed (System.Collections.Hashtable, mine.log, CorpusScan/
     CorpusSurvey csv); TechTalk docs MOVED to Downloads\TechTalk (not PB deliverables).
     MAINTENANCE: files\ and GPF-PackageBuilder are separate codebases now - port engine fixes manually.
     GPF remaining: Step-1 GUI wiring of the request browser (engine Resolve-GpfRequest done+tested), ModulePack xlsx
     auto-fill, their real share paths. See memory gpf-brand-variant.md.
   2026-07-10 LIB LAYOUT (user rule "keep all modules and templates in lib only"): moved PSADT_Template\ and
