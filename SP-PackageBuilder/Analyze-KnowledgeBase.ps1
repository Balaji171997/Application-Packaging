...
function Get-PackageFacts {
    ...
    $id = Parse-Name $name
    # If the FOLDER name doesn't follow our Vendor_App_Arch_Version-Release_Lang convention (another repo's
    # packages), take identity from the SCRIPT itself ($adtSession / $app* fields) - so the KB still records the
    # right vendor/app/version. Folder values fill any gaps the script doesn't define.
    if (-not $id.Matched) {
