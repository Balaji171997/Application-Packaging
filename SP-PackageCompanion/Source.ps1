...
# Validate the chosen installer against the package name. Returns @(review strings) - version cross-check
# (installer vs package name) and architecture cross-check. Surfaced in Review. (The digital-signature line was
# removed on request: sources come from the vendor via the team's own intake, so it was noise on every package.)
...
# 'sw-source' & co. are the MAN layout (PackageSources on SharePoint and the older shares): without them the
# resolver drops to scan mode, Files\ gets the whole 'SW-Source\...' folder instead of its contents, and every
# license .txt under it is swept into Documents.
$script:SourceNames   = @('source','sources','src','vendor source','vendorsource','vendor_source','sw-source','sw source','swsource','sw_source')
...
# Copy doc items into the package's Documents\. A folder that IS the documents folder (named doc / Documents / ...)
# contributes its CONTENTS - the packager expects the .docx straight in Documents\, not Documents\Documents\.docx.
# Any other folder carried across (e.g. 'Order Info') keeps its name, so it stays recognisable; files copy as-is.
function Copy-DocItems {
    param([string[]]$Items, [Parameter(Mandatory)][string]$Dest)
    ...
    foreach ($item in @($Items)) {
        ...
        $isDocFolder = (Test-Path -LiteralPath $item -PathType Container) -and ($script:DocNames -icontains (Split-Path $item -Leaf))
        if ($isDocFolder) {
            if (@(Get-ChildItem -LiteralPath $item -Force -ErrorAction SilentlyContinue).Count) { Copy-Item -Path "$item\*" -Destination $Dest -Recurse -Force }
        } else {
...
function Copy-ResolvedSource {
    ...
    Copy-DocItems -Items @($Resolved.DocItems) -Dest $DocDest
