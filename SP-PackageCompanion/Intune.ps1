...
function Get-IntuneDetectionRules {
    ...
    if ($type -eq 'ProductCode') {
        ...
        $vn = if ("$($Fields.DetectValueName)".Trim()) { "$($Fields.DetectValueName)".Trim() } else { 'DisplayVersion' }
        $r = @{ '@odata.type'='#microsoft.graph.win32LobAppRegistryDetection'
                keyPath="HKEY_LOCAL_MACHINE\$($Fields.UninstallKey)"; valueName=$vn
...
function Get-IntuneUninstallSignature {
    ...
    $vn = if ("$($Fields.DetectValueName)".Trim()) { "$($Fields.DetectValueName)".Trim() } else { 'DisplayVersion' }
    return @{ KeyPath=$keyPath; ValueName=$vn; Is32Bit=[bool]$Fields.Is32Bit; Version="$($Fields.DetectVersion)".Trim(); ProductCode="$($Fields.ProductCode)".Trim() }
}
