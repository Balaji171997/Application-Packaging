...
#   border #2F343D (soft)
#   TEXT TIERS (all readable on the dark surfaces - do not invent new greys):
#     primary   #E7E9ED  field labels, values, anything the user acts on
#     secondary #B7BEC8  hints and explanations under a control        (~9.5:1)
#     tertiary  #A0A8B4  fine print: examples, bottom strip, placeholder (~7:1)
#     (deliberately bright: users run dimmed monitors - anything under ~#A0 disappears)
#     disabled  #6A717C  greyed-out controls only - never for readable text
...
<ResourceDictionary xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
  ...
  <SolidColorBrush x:Key="PbText2"     Color="#FFB7BEC8"/>
  <SolidColorBrush x:Key="PbMuted"     Color="#FFA0A8B4"/>
  ...
  <!-- ===== Selectable text (keyed): a read-only, borderless TextBox that LOOKS like a TextBlock but can be
       selected and copied with the mouse. Use for every label whose text a packager may need to paste
       somewhere (package name, RITM, product code, paths, results). Reference with DynamicResource - the
       theme is merged AFTER the window XAML loads, so StaticResource would fail. ===== -->
  <Style x:Key="PbCopyText" TargetType="TextBox">
    ...
    <Setter Property="Template">
      <Setter.Value>
        <ControlTemplate TargetType="TextBox">
          <ScrollViewer x:Name="PART_ContentHost" Padding="0" Margin="0" Background="Transparent"
  ...
  <!-- ===== Glyphs + page sections (keyed, DynamicResource) =====
       Glyphs come from Segoe MDL2 Assets, which every Windows 10/11 machine ships - no image files, no
       licensing, crisp at any DPI. A glyph inherits Foreground so it follows the button/pill state.
       A SECTION is title row (glyph + name) + one-line description + hairline; content sits under it.
       No boxes: sections are separated by rhythm, not by cards. -->
  <Style x:Key="PbGlyph" TargetType="TextBlock">
    <Setter Property="FontFamily" Value="Segoe MDL2 Assets"/>
    <Setter Property="FontSize" Value="13"/>
    <Setter Property="VerticalAlignment" Value="Center"/>
    <Setter Property="Margin" Value="0,1,7,0"/>
  </Style>
  <Style x:Key="PbSectionTitle" TargetType="TextBlock">
    <Setter Property="FontSize" Value="13.5"/>
    <Setter Property="FontWeight" Value="SemiBold"/>
    <Setter Property="Foreground" Value="{StaticResource PbText}"/>
    <Setter Property="VerticalAlignment" Value="Center"/>
  </Style>
  <Style x:Key="PbSectionDesc" TargetType="TextBlock">
    <Setter Property="FontSize" Value="11.5"/>
    <Setter Property="Foreground" Value="{StaticResource PbText2}"/>
    <Setter Property="TextWrapping" Value="Wrap"/>
    <Setter Property="Margin" Value="0,3,0,0"/>
  </Style>
  <Style x:Key="PbSectionRule" TargetType="Border">
    <Setter Property="BorderBrush" Value="#FF2A2F38"/>
