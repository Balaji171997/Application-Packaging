...
function Get-TemplateRoot {
    ...
    foreach ($cand in @((Join-Path $Root 'PSADT_Template'), (Join-Path $Root 'Lib\PSADT_Template'))) {
...
function Convert-IcoToPng {
    ...
    # PRIMARY: WPF BitmapDecoder - handles .ico with PNG-compressed frames AND classic BMP frames, and picks the
    # largest frame. (System.Drawing.Icon.ToBitmap() CHOKES on PNG-compressed frames - the reported failure - so it
    # is only the fallback for the rare case WPF isn't available.)
    try {
        ...
        try {
            ...
            $out = [IO.File]::Open($PngPath, [IO.FileMode]::Create); try { $enc.Save($out) } finally { $out.Close() }
            return $true
        } finally { $fs.Close() }
    } catch {
        # FALLBACK: System.Drawing (works for classic BMP-frame .ico only).
        try {
...
# Icon-readiness gate - run BEFORE SCCM create / Intune create / Intune update-content (user rule). Guarantees the app
# gets an icon and that ARP/SCCM (.ico) and Intune (.png) show the SAME image:
#   - a .png already present -> ready.
#   - only .ico present      -> CONVERT it to a persisted .png (same basename) so the upload never silently skips it.
#   - neither                -> NOT ready: the caller must BLOCK and tell the user to add an .ico first.
# Returns @{ Ready=[bool]; Message=<text>; Converted=[bool]; IconsDir=<path> }.
function Confirm-PackageIconReady {
    param([Parameter(Mandatory)][string]$PackagePath)
    if (-not "$PackagePath".Trim() -or -not (Test-Path -LiteralPath $PackagePath)) {
        return @{ Ready=$false; Message="Package path not found: $PackagePath" }
    }
    $iconsDir = Join-Path $PackagePath 'Icons'
    if (-not (Test-Path -LiteralPath $iconsDir)) {
        $up = Split-Path $PackagePath -Parent   # caller may pass the Content folder instead of the package root
        if ($up -and (Test-Path -LiteralPath (Join-Path $up 'Icons'))) { $iconsDir = Join-Path $up 'Icons' }
    }
    $ico = $null
    if (Test-Path -LiteralPath $iconsDir) {
        if (Get-ChildItem -LiteralPath $iconsDir -File -Filter *.png -ErrorAction SilentlyContinue | Select-Object -First 1) {
            return @{ Ready=$true; Message=''; Converted=$false; IconsDir=$iconsDir }
        }
        $ico = Get-ChildItem -LiteralPath $iconsDir -File -Filter *.ico -ErrorAction SilentlyContinue | Select-Object -First 1
    }
    if ($ico) {
        $target = Join-Path $iconsDir ($ico.BaseName + '.png')
        if (Convert-IcoToPng -IcoPath $ico.FullName -PngPath $target) {
            if (Get-Command Write-Log -ErrorAction SilentlyContinue) { Write-Log "Icon: no PNG present - converted $($ico.Name) -> $([IO.Path]::GetFileName($target)) for the app icon." Success }
            return @{ Ready=$true; Message="Converted $($ico.Name) to PNG."; Converted=$true; IconsDir=$iconsDir }
        }
        return @{ Ready=$false; Message="An .ico exists ($($ico.Name)) but converting it to a .png failed. Put a valid .ico (or a .png) in:`n$iconsDir`nthen retry."; IconsDir=$iconsDir }
    }
    $where = if (Test-Path -LiteralPath $iconsDir) { $iconsDir } else { Join-Path $PackagePath 'Icons' }
    return @{ Ready=$false; Message="No icon (.ico or .png) found for this package - the installer provided none. Copy an .ico file into:`n$where`nthen retry the create / update."; IconsDir=$where }
}
...
function New-Package {
    ...
    if ($LooseFiles) {
        ...
        if ($Resolved) {
            Copy-DocItems -Items @($Resolved.DocItems) -Dest $docsDir   # same flattening rule as Copy-ResolvedSource
