...
function New-SccmUninstallClause {
    ...
    $vn = if ("$($Fields.DetectValueName)".Trim()) { "$($Fields.DetectValueName)".Trim() } else { 'DisplayVersion' }   # any value under the key, DisplayVersion by default
    $p = @{ ExpectedValue=$Fields.DetectVersion; Hive='LocalMachine'; KeyName=$Fields.UninstallKey; Value=$true; ValueName=$vn }
...
function Get-SccmDetection {
    ...
    try {
        ...
        $det = [ordered]@{ DetectType='None'; UninstallKey=''; DetectVersion=''; DetectValueName='DisplayVersion'; Is32Bit=$false; ProductCode='' }
        foreach ($c in $clauses) {
            ...
            if ($src -match '(?i)MSI|Installer|ProductCode') {
                ...
                if ("$($c.Setting.ValueName)".Trim()) { $det.DetectValueName = "$($c.Setting.ValueName)" }
